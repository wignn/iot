...
void setup() {
  Serial.begin(115200);
  Serial.println();
  Serial.println("LoRa OLED Receiver - ESP32");

  Wire.begin(OLED_SDA_PIN, OLED_SCL_PIN);
  oledAvailable = display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDRESS);
  if (oledAvailable) {
    showMessage("LoRa receiver", "Initializing...");
  } else {
    Serial.println("OLED not detected at I2C address 0x3C.");
  }

  SPI.begin(LORA_SCK_PIN, LORA_MISO_PIN, LORA_MOSI_PIN, LORA_NSS_PIN);
  LoRa.setPins(LORA_NSS_PIN, LORA_RST_PIN, LORA_DIO0_PIN);
  if (!LoRa.begin(LORA_FREQUENCY)) {
    Serial.println("LoRa initialization failed. Check power and wiring.");
    showMessage("LoRa failed", "Check wiring");
    while (true) {
      delay(1000);
    }
  }
  ...
  LoRa.receive();
  showMessage("LoRa ready: 433MHz", "Waiting for packets");
